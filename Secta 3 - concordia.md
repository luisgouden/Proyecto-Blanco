### @race.concordia — Concordia (Concordia del Prisma / “Liga del Alba Serena”)
...
- **Tres axiomas**:
  1) Toda obediencia empieza por un gusto (algo que el sujeto pide repetir).
  2) Todo gusto se vuelve deuda si alguien lo registra (testigos, recibos, listas, cartas).
  3) Toda deuda se vuelve verdad cuando deja cicatriz (social o física).

#### 3) Dogma mínimo (portable; idéntico en todos los cantones)
1) El Colapso fue **apertura** (sobreexposición del mundo).
2) La verdad no es una: es **espectro** (ángulo, medio, filtro).
3) Gobernar es controlar el **instrumento óptico** (qué iluminas y qué dejas fuera).
4) El placer es la vía más barata hacia obediencia (deuda social).
5) Si el placer falla, se sella con costo (ruina, sangre, vergüenza o tiempo).
6) Lo no registrado cae en sombra: **no existe** para el poder.
7) El “mundo blanco” es vestíbulo: **peaje, monopolio, cartografía**.

#### 4) Organización política y social
- **Forma de gobierno**: confederación clandestina de **cantones** autónomos; compiten y cooperan por interés. La unidad es **protocolaria**, no teológica.
- **Cantones (núcleos)**:
  - **La Lente (Teeli)**: enfoque/definición (currículos, bibliotecas, traducciones canónicas).
  - **La Incandescencia (Orka)**: costo/violencia como virtud (juramentos, élites marciales, guerras pequeñas).
  - **La Bruma (Henso)**: atenuación/laberinto (mensajería, contadurías, retrasos y ruido).
  - **El Espectro (Mieru)**: refracción social (identidades, reputaciones, ocio elegante, captación).
- **Órganos transversales (obligatorios)**:
  - **Oficio del Prisma**: sellos, claves, papel, tintas, marcas de agua; decide qué “emite luz” oficial.
  - **Cámara de Incidencia**: reparto de riesgo y culpas; asigna firmas, chivos expiatorios, guiones de crisis.
...
- **Camino del Foco (Instrumentalistas)**  
  Tesis: la verdad es lo que funciona para producir obediencia estable.  
  Método: educación, definiciones, reglamentos, “descubrimientos” plantados.  
  Bastiones: La Lente, Oficio del Prisma.  
  Debilidad: ceguera por exceso de control; arrogancia del modelo.
...
- **Camino del Prisma (Relativistas doctrinales)**  
  Tesis: dominar es refractar: dar a cada grupo su color y comprar su complicidad.  
  Método: relatos múltiples compatibles, identidades compradas, reputaciones fabricadas, ocio como reclutamiento.  
  Bastiones: El Espectro, Casa del Resplandor.  
  Debilidad: dependencia de respetabilidad; si cae, la operación se encarece.

- **Camino del Blanco (Puristas / Teleólogos)**  
  Tesis: el espectro conduce a una “luz blanca” final; no todos merecen verla.  
  Método: cartografía, pruebas oraculares (condiciones/coincidencias), expediciones pantalla.  
  Bastiones: Sala Oscura, Cartografía del Deslumbramiento.  
  Debilidad: obsesión por señales; cazables con falsos destellos; queman recursos por lecturas.

#### 7) Gobernanza del conflicto interno
- **Sínodo de Longitudes** (subórgano de la Sala Oscura): arbitra cuando chocan los Caminos. No decide “verdad”; decide tolerancias (ruido, exposición pública, sangre).
- **Edicto de Calibración**:
  1) Si dos Caminos pueden coexistir, coexisten.
  2) Si se estorban, manda el Oficio del Prisma (sin documentos no hay mundo).
  3) Si hay riesgo existencial, manda el Haz (el cierre vale más que la elegancia).
  4) Si hay oportunidad de monopolio del vestíbulo blanco, manda el Blanco (premio estratégico).
...
#### 9) Economía y control
- **Moneda real**: el favor registrable (coartadas, recibos, listas, testimonios).
- **Activos estratégicos**: sellos/claves/papel (Oficio), reputación (Espectro), guiones de crisis (Incidencia), peajes y rutas del mundo blanco (Cartografía).
- **Método estándar de dominación**:
  1) Seducir (placer / pertenencia).
  2) Registrar (deuda / testigo / documento).
  3) Marcar (costo: ruina, sangre, vergüenza o tiempo).
  4) Certificar (archivo: “lo ocurrido” queda fijado).

#### 10) Limitaciones (antiómnipotencia) y grietas explotables
- **Dependencia del Oficio del Prisma**: si pierden sellos/papel/claves, caen a conspiración local.
- **Guerra interna de Caminos**: Foco desprecia al Haz; Haz al Prisma; Blanco a todos. Ese roce deja grietas.
- **Sesgo de señales (Blanco)**: son cazables con falsos patrones.
...
5) **Peregrinos del Vestíbulo**: expediciones y mapas; brazo público del Blanco.
